...
if(~shade)
    ...
    if (size(EE,1)==1 || size(EE,2)==1) % symmetric error
        ...
        EE=EE(:);
        YY=YY(:);
        h=fill(XX([1:end end:-1:1]),[YY(1:end)-EE(1:end); YY(end:-1:1)+EE(end:-1:1)]',col,'FaceAlpha',0.5,'EdgeAlpha',0.5);
    else % non-symmetric c.i.
        h=fill(XX([1:end end:-1:1]),[EE(1:end,1); EE(end:-1:1,2)],col,'FaceAlpha',0.5,'EdgeAlpha',0.5);
    end
    set(h,'edgecolor',col);
end
